...
Review the planned actions and detected drift.

Then execute:

uv run github-bootstrap sync

Finally verify the resulting Project V2 and repository resources in GitHub.
...
As a result, existing issues may appear as synchronization actions even when their values already match.

Project repository linking

Issues can be added to a Project V2, but the Project itself is not currently linked automatically to the repository Projects tab.
